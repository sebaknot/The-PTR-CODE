...
router.post("/deliveries", async (req, res): Promise<void> => {
  ...
  const [delivery] = await db
    ...
    .values({
      // Always the authenticated user — never trust a sender id from the body.
...
router.post("/deliveries/:id/accept", async (req, res): Promise<void> => {
  ...
  if (requestingUser.role !== "courier") {
    res.status(403).json({ error: "Only porters can accept deliveries" });
    return;
  }
  ...
  // Atomic claim: only succeeds if the delivery is still acceptable AND unclaimed.
  // Two porters racing to accept the same job — only one UPDATE matches a row.
  const claimable =
    existing[0].status === "pending"
      ? and(eq(deliveriesTable.id, id), eq(deliveriesTable.status, "pending"))
      : and(
  ...
  const claimed = await db
    ...
    .set({
      // Courier is always the authenticated user — never trusted from the body.
      courierId: requestingUser.id,
      status: "accepted",
      acceptedAt: new Date(),
      updatedAt: new Date(),
    })
...
router.patch("/deliveries/:id/status", async (req, res): Promise<void> => {
  ...
  // Authorization: only a participant may change a delivery's status, and the
  // fulfillment transitions (picked_up / in_box / delivered) are the assigned
  // porter's alone. The sender may only cancel their own delivery.
  ...
  const isCourier = current.courierId === requestingUserId;
  const isSender = current.senderId === requestingUserId;
  ...
  if (status === "cancelled") {
    ...
    res.status(403).json({ error: "Only the assigned porter can update this delivery" });
    return;
  }
...
router.post("/deliveries/:id/rate", async (req, res): Promise<void> => {
  ...
  // Only the sender of this delivery may rate it, and only once it's delivered.
  if (delivery.senderId !== requestingUserId) {
    res.status(403).json({ error: "Only the package sender can rate this delivery" });
    return;
  }
  if (delivery.status !== "delivered") {
    res.status(409).json({ error: "Only delivered packages can be rated" });
    return;
  }
