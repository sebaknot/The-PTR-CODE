...
// Validate the untrusted delivery payload before it drives price calculation and
// Stripe metadata. Coordinates are bounded; strings are length-capped.
const DeliveryDataSchema = z.object({
  ...
  deliveryType: z.enum(["standard", "porter_box", "box_dropoff"]).optional(),
  porterBoxId: z.string().max(100).optional(),
  senderPhotoUrl: z.string().max(2000).optional(),
});
type DeliveryData = z.infer<typeof DeliveryDataSchema>;
...
router.post("/payments/intent", async (req, res): Promise<void> => {
  ...
  const parsedBody = DeliveryDataSchema.safeParse(
    (req.body as { deliveryData?: unknown })?.deliveryData,
  );
  if (!parsedBody.success) {
    res.status(400).json({ error: parsedBody.error.message });
    return;
  }
  const deliveryData: DeliveryData = parsedBody.data;
