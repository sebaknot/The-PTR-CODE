...
// The demo seeder creates rows and starts background simulations, so it must not
// be openly callable. It is disabled unless DEMO_SECRET is set, and then requires
// a matching `x-demo-secret` header. In production without the secret, it's off.
function requireDemoAccess(req: Request, res: Response, next: NextFunction): void {
...
router.use("/demo", requireDemoAccess);
